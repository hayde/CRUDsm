...
public abstract class CRUDsm<CRUDID, CRUDObject> {
	...
	protected CRUDEvents		currentEvent = null;
	...
	/**
	 * the constructor, which expects the objects in hashmap format to reach
	 * the elements quite fast
	 * 
	...
	/**
	 * does set the model listener, that is responsible for questioning the 
	 * user if he wants to delete or save the entry he is currently working with
	 * 
	 * @param modelListener 
	 */
		...
		if( internalRefreshState ) {
			...
			try {
				...
				switch( newEvent ) {
					...
					case SELECT:		roadmap.add( stateMachine.PRESELECTED );
										if( changeFlag ) {
											roadmap.add( stateMachine.AUTOMATICSAVE );
										}
					...
					case DELETE:		roadmap.add( stateMachine.DELETE );
										roadmap.add( stateMachine.NOTHING_SELECTED );
										roadmap.add( stateMachine.REFRESHVIEW );
										break;

					case CHANGED:		if( ! changeFlag ) {
											roadmap.add( stateMachine.CHANGED );
										}

										break;
					case CLOSING:		if( changeFlag ) {
											roadmap.add( stateMachine.AUTOMATICSAVE );
											roadmap.add( stateMachine.REFRESHVIEW);
										}
										break;
	...
	private boolean process( List<stateMachine> roadmap ) {
		...
		/**
		 * we put this variable for true so long, till a "false" is given 
		 * back from one of the actionFunctions.
		 */
		...
		while ( (roadmap.size() > 0) && returnValue ) {
			...
			switch( roadmap.get( 0 ) ) {
				...
				case INITIALIZE:	this.objects = actionInit();
									if( objects == null ) {
										returnValue = false;
									} else {
										returnValue = true;
				...
				case PRESELECTED:	selectedButNotCurrentID = actionSelected();
									break;

				case SELECTED:		currentID=selectedButNotCurrentID;
				...
				case CHANGED:		changeFlag = true;
									actionChanged();
									returnValue = true;
									break;
					
				case SAVE:			{
										currentID = actionSave();
										if( currentID == null ) {
											returnValue = false;
										} else {
											returnValue = true;
											...
											if( objects.containsKey(currentID) ) {
												// already existing in map, 
												// so do nothing
											} else {
												// this element is new to the system
												// so put it it
												objects.put(currentID, currentElement);
				...
				case AUTOMATICSAVE:
									if (this.bAutomaticSaveActivated ){
										...
										} else if( saveIt == CRUD_ACTION_AUTOMATED_SAVE.CANCEL ) {
											returnValue = false;

										} else if( saveIt == CRUD_ACTION_AUTOMATED_SAVE.DISCARD ) {
											changeFlag = false;
											returnValue = true;
										}
									}
									break;
				case DELETE:		if ( actionDeleteOrCancel() != CRUD_ACTION_DELETE_OR_CANCEL.DELETE )
									{
										returnValue = false;
									} else {
										if( currentID == null ) {
				...
				case REFRESHVIEW:	actionRefreshView();
									break;
				
				case VALIDATING:	if( actionValidate( this.currentElement ) ) {
										returnValue = true;
									} else {
										returnValue = false;
									}
	 ...
	 *
	 * In general you create here a new working object, where you put all
	 * the information into. You also enable the save buttons and disable the
	 * delete button.
	 */
	 ...
	 * already asked, if he want to perform the delete or cancel the action.
	 * He already decided to delete.
	 *
	 ...
	 */
	public abstract CRUDID actionSave();
